...
export async function deleteBooking(bookingId) {
  ...
  if (!guestBookingIds.includes(bookingId)) throw new Error("You are not alowed to delete this booking!");
...
export async function updateBooking(formData) {
  ...
  if (!bookingsIds.includes(bookingId)) throw new Error("You are not allowed to edit this booking!");
  ...
  const { error } = await supabase.from("bookings").update(updatedFields).eq("id", bookingId).select().single();
